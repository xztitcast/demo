<template>
  <main :class="['site-content', { 'site-content--tabs': $route.meta.isTab }]">
    <!-- tab展示内容 -->
    <template v-if="$route.meta.isTab">
      <el-dropdown class="site-content--tabs-tools">
        <i class="el-icon-arrow-down"></i>
        <template v-slot:dropdown>
          <el-dropdown-menu :show-timeout="0">
            <el-dropdown-item @click.enter="tabRemoveHandle($store.state.contentTabsActiveName)">{{ $t('contentTabs.closeCurrent') }}</el-dropdown-item>
            <el-dropdown-item @click.enter="tabsCloseOtherHandle()">{{ $t('contentTabs.closeOther') }}</el-dropdown-item>
            <el-dropdown-item @click.enter="tabsCloseAllHandle()">{{ $t('contentTabs.closeAll') }}</el-dropdown-item>
          </el-dropdown-menu>
        </template>
      </el-dropdown>
      <el-tabs v-model="$store.state.contentTabsActiveName" @tab-click="tabSelectedHandle" @tab-remove="tabRemoveHandle">
        <el-tab-pane
          ...
          <template v-if="item.name === 'home'" v-slot:label>
            <svg class="icon-svg site-content--tabs-icon-nav" aria-hidden="true"><use xlink:href="#icon-home"></use></svg>
          </template>
